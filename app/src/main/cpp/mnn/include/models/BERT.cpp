...
namespace MNN {
    namespace Train {
        namespace Model {
            ...
            std::vector<Express::VARP> BERTPooler::onForward(const std::vector<Express::VARP> &inputs) {
                ...
                auto firstTokenTensor = _Slice(hiddenStates, _Const(startSlice, {3}, NCHW, halide_type_of<int>()), _Const(sizeSlice, {3}, NCHW,
                                                                                                                                 halide_type_of<int>()));
            ...
            BERTEncoder::BERTEncoder(int numHiddenLayers, int numAttentionHeads, int hiddenSize, int intermediateSize, float dropoutProb, bool forParallel) {
                for (int i = 0; i < numHiddenLayers; i++) {
                    layer.push_back(std::make_shared<BERTLayer>(numAttentionHeads, hiddenSize, intermediateSize, dropoutProb, forParallel));
                }

                registerModel(layer);
            }

            std::vector<Express::VARP> BERTEncoder::onForward(const std::vector<Express::VARP> &inputs) {
                using namespace Express;
                VARP hiddenStates = inputs[0];
            ...
            BERT::BERT(int vocabSize, int hidden, int nLayers, int attnHeads, int intermediateSize, float dropout, bool forParallel) {
                this->embedding = std::make_shared<BERTEmbedding>(vocabSize, hidden, 512, 2, dropout);
                this->encoder = std::make_shared<BERTEncoder>(nLayers, attnHeads, hidden, intermediateSize, dropout, forParallel);
            ...
            std::vector<Express::VARP> BERT::onForward(const std::vector<Express::VARP> &inputs) {
                ...
                if (inputs.size() < 2) {
                    ...
                    attentionMask = _Const(maskVec.data(), inputIds->getInfo()->dim, NCHW, halide_type_of<float>());
                } else {
                ...
                if (inputs.size() < 3) {
                    ...
                    tokenTypeIds = _Const(allZeros.data(), inputIds->getInfo()->dim, NCHW, halide_type_of<int>());
                } else {
                ...
                auto encoderOutput = encoder->onForward({embeddingOutput[0], extendedAttentionMask});
            ...
            BERTForClassification::BERTForClassification(int vocabSize, int hidden, int nLayers, int attnHeads,
                                                         int intermediateSize, float attentionDropout,
                                                         float hiddenDropout, int numClasses, bool forParallel) {
                this->nLayers = nLayers;

                this->bert = std::make_shared<BERT>(vocabSize, hidden, nLayers, attnHeads, intermediateSize, attentionDropout, forParallel);
            ...
            std::vector<Express::VARP> BERTForClassification::onForward(const std::vector<Express::VARP> &inputs) {
            ...
            SubBERTForClassification::SubBERTForClassification(int start, int end, std::map<std::string, double>& args) {
                ...
                if (start == 0) {
                    this->embedding = std::make_shared<BERTEmbedding>(vocabSize, hidden, 512, 2, attentionDropout);
                ...
                for (int i = 0; i < numHiddenLayers; i++) {
                    if (curLayer >= start && curLayer <= end) {
                        encoders.push_back(std::make_shared<BERTLayer>(numAttentionHeads, hidden, intermediateSize, attentionDropout));
                    }
                    curLayer++;
                }
            ...
            std::vector<Express::VARP> SubBERTForClassification::onForward(const std::vector<Express::VARP> &inputs) {
                using namespace Express;
                VARP x = inputs[0];
                ...
                if (embedding != nullptr) {
                    ...
                    if (inputs.size() < 3) {
                        ...
                        tokenTypeIds = _Const(allZeros.data(), x->getInfo()->dim, NCHW, halide_type_of<int>());
                    } else {
                        tokenTypeIds = inputs[2];
                    }
                ...
                if (!encoders.empty()) {
                    ...
                    if (inputs.size() < 2) {
                        ...
                        attentionMask = _Const(maskVec.data(), x->getInfo()->dim, NCHW, halide_type_of<float>());
                    } else {
                        attentionMask = inputs[1];
                    }
...
//                }
            ...
            void SubBERTForClassification::loadParamByLayer(int layer, int startLayer) {
                ...
                std::string path = "../../pretrained_weights/bert/bert_" + std::to_string(layer) + ".mnn";
